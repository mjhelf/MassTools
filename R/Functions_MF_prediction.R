...
#'
getSenior3 <- function(MF, vals = c(C = 4, H = 1,
                                    Br = 1, Cl = 1,
                                    F = 1, N = 5,
                                    O = 2, P = 5,
                                    S = 6, Si = 4,
...
calcMF <- function(mz = 200.000659,
  ...
  sfs <- makeMF(res$MF, forcelist = T)
  
  ret <- sapply(sfs, function(sf){
    
if(!is.null(Filters$maxElements)){
    maxmatch <- match(names(sf),names(Filters$maxElements))
    
    if(any(sf[!is.na(maxmatch)] > Filters$maxElements[na.omit(maxmatch)])){return(FALSE)}
}
  ...
  if(maxCounts){
    ...
    if(!is.null(maxCountLimit)){
      ...
      res$maxCounts <- sapply(sfs, function(sf){
          ...
          maxmatch <- match(names(sf),names(maxCountLimit))
          #print(maxmatch)
          if(any(sf[!is.na(maxmatch)] > maxCountLimit[na.omit(maxmatch)])){return(FALSE)}
